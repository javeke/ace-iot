...
const char* softAPPassword = "Tester123";
...
void setup() {
  ...
  displayOnOLED(1000, screenToolbar, F("Mounting LittleFS"));

  if(!LittleFS.begin()){
    displayOnOLED(1000, screenToolbar, F("Failed to mount LittleFS"));
  }
  else {
    displayOnOLED(1000, screenToolbar, F("Mounted LittleFS"));
...
void displayOnOLED(int delayTime, String toolbar, String secondLine, String thirdLine, String fourthLine){
  display.clearDisplay();
...
void readMPU(){
  if(millis() > (unsigned long)(MPU_TIMEOUT + mpuInterval)){
...
void handleHomeRoute() {
  String message = "Welcome to Ace for device ";
...
void setUpServerRoutes(){
  ...
  httpServer.serveStatic("/", LittleFS, "/", "no-cache");
}
